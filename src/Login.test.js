...
describe("Login component Tests", () => {
  ...
  it("displays two input boxes and 1 submit button initially", () => {
    expect(emailInput).toHaveLength(1);
    expect(passwordInput).toHaveLength(1);
    expect(wrapper.find("button")).toHaveLength(1);
  });

  it("Displays error message when email is empty", () => {
  ...
  it("Displays error messages when both email and password are empty", () => {
    passwordInput.simulate('change', { target: { value: '' } });
    form.simulate('submit')
    expect(wrapper.find("#passwordError")).toHaveLength(1);
    expect(wrapper.find("#emailError")).toHaveLength(1);
